...
const cardMap = {
  ...
  EvAdain: "EvAdain.png",
  JerryCraft: "JerryCraft.png",
  "JustJosh Gaming": "JustJoshGaming.png",
  Kooby: "Kooby.png",
  MonkeyingAround: "MonkeyingAround.png",
  Remix: "Remix.png",
  Sab: "Sab.png",
  "Sarah Jo": "SarahJo.png",
  Secluded: "Secluded.png",
  Seth: "Seth.png",
  SkepticHailz: "SkepticHailz.png",
  "Tom Frizzle": "TomFrizzle.png",
  Twizza: "Twizza.png",
  "Wya Benny": "WyaBenny.png",
};
...
function getWeekNumber() {
  const now = new Date();
  const start = new Date(2026, 0, 1); // pick a fixed start date
  const diff = now - start;
  return Math.floor(diff / (1000 * 60 * 60 * 24 * 7));
}

function getShuffledOrder() {
  let savedOrder = localStorage.getItem("creatorOrder");

  if (!savedOrder) {
    // Create new shuffled list starting with Craftopia
    const startName = "Craftopia";
    const others = members.filter(m => m.name !== startName);
    ...
    const fullOrder = [
      members.find(m => m.name === startName),
      ...others
    ];

    localStorage.setItem("creatorOrder", JSON.stringify(fullOrder));
    return fullOrder;
  }
...
function displayCreatorOfWeek() {
  ...
  // Sequential rotation starting from Craftopia
  const startCreatorName = "Craftopia";
  const startIndex = members.findIndex((member) => member.name === startCreatorName);
  const creatorIndex = (weekNumber - 1 + (startIndex >= 0 ? startIndex : 0)) % members.length;
  const order = getShuffledOrder();
  const creatorOfWeek = order[creatorIndex];

  const cardFile = cardMap[creatorOfWeek.name] || "";
  const cardImage = cardFile ? `assets/Creator_Cards/${cardFile}` : "";
  ...
  document.getElementById('creator-skin').alt = `${creatorOfWeek.name} Creator Card`;
